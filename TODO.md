...
- [ ] Refactor the AST-loading responsibility out of `main.py` into `project_loader`, since the current note says it acts more like a loader.
- [ ] Remove or replace the hardcoded debug project path in `main.py`.
- [ ] Add recovery-aware state usage in `main.py` so saved state is used after crashes, not as the default path.
...
- [ ] Consider splitting the top-level workflow in `main.py` into smaller phases/modules.
...
- [ ] Optionally evaluate Redis for faster state storage if external dependencies become acceptable in `state_storage.py`.
...
- [ ] Make the root project path handling absolute in `project_loader/file_discover.py`.
...
- [ ] Rename or privatize the dataclass helper in `analyze/ast_parser.py` as intended.
- [ ] Revisit the AST parser design in `analyze/ast_parser.py`: it currently mixes graph generation with code clipping responsibilities.
...
- [ ] Improve hotspot scoring in `analyze/hotspot_detector.py`, which is currently just `max_nesting + max_conditionals`.
...
- [ ] Keep hotspot data derived from AST state rather than persisted separately, per `analyze/hotspot_detector.py`.
